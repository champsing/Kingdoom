...
execute as @e[tag=edit] at @s if data entity @s ArmorItems[3].tag.edit run function edit:head
execute as @e[type=item,nbt={Item:{id:"minecraft:player_head",tag:{BuildScore:1}}}] at @s if entity @e[tag=build,limit=1,distance=..1] run function edit:score/count
...
execute as @a[nbt={SelectedItem:{id:"minecraft:written_book",tag:{name:1}}}] run function edit:build/check_name
