...
execute as @e[type=item] if items entity @s contents player_head[custom_data~{BuildScore:1}] at @s if entity @e[tag=build,limit=1,distance=..1] run function edit:score/count
...
execute as @a if items entity @s weapon.mainhand written_book[custom_data~{name:1}] run function edit:build/check_name
